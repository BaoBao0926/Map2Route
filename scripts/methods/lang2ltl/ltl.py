"""Small LTLf formula library used by the Lang2LTL adapter."""
...
class Formula:
    ...
    @property
    def hash(self) -> int:
        """Compatibility alias for planners that use ``formula.hash`` as a key.

        ``Formula`` is an immutable dataclass, so its structural Python hash is
        the correct identifier for in-memory search state. Keeping this as a
        property also prevents a stale, separately maintained hash value.
        """
        return hash(self)
