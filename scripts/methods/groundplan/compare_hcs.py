"""Compare paired HCS values for GroundPlan full and ToolCall outputs."""
...
DEFAULT_FULL_ROOT = REPO_ROOT / "resources" / "methods" / "groundplan" / "main_result"
DEFAULT_TOOL_ROOT = REPO_ROOT / "resources" / "methods" / "groundplan" / "ablation" / "01_grounding_strategy" / "tool_call"
...
def compare_hcs(full_root: Path, tool_root: Path) -> dict[str, object]:
    ...
    def aggregate(items: list[dict[str, object]]) -> dict[str, object]:
        ...
        return {
            ...
            "groundplan_full_hcs": mean(full_values),
    ...
    return {
        "groundplan_full_root": str(full_root),
        "tool_call_root": str(tool_root),
        "available_records": {"groundplan_full": len(full), "tool_call": len(tool)},
...
def _print_table(report: Mapping[str, object]) -> None:
    ...
    print(
        "available: "
        f"GroundPlan-full={available['groundplan_full']} "
        f"ToolCall={available['tool_call']}"
    )
    print("split\tpaired\tGroundPlan-full\tToolCall\tToolCall-full\tToolCall W/T/L")
    for split in ("overall", "easy", "hard"):
        row = report[split]
        assert isinstance(row, Mapping)
        outcomes = row["tool_win_tie_loss"]
        assert isinstance(outcomes, Mapping)
        full_hcs = row["groundplan_full_hcs"]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare GroundPlan full and ToolCall HCS on their shared episodes."
    )
