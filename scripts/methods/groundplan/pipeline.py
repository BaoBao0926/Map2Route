"""End-to-end GroundPlan pipeline."""
...
from scripts.methods.groundplan.alias_resolver import (
    resolve_unknown_entity_alias,
    unknown_entity_category_from_failure,
)
from scripts.methods.groundplan.grounding import SceneMap, ground_program
from scripts.methods.groundplan.grounding.code import (
    GroundingCodeError,
...
from scripts.methods.groundplan.parse import (
    parse_instruction,
    scene_category_summary,
    supported_categories,
)
from scripts.methods.groundplan.parse.api_program import llm_repair_api_after_grounding_failure
from scripts.methods.groundplan.parse.intent import llm_repair_intent_after_grounding_failure
from scripts.methods.groundplan.planner import plan_grounded_program
from scripts.methods.groundplan.constraint_evaluation import apply_scope_ablation
from scripts.methods.groundplan.ir import GPProgramSpec, GroundedProgram, PlannedProgram


@dataclass(frozen=True)
class GroundPlanRunConfig:
...
def resolve_grounding_architecture(config: GroundPlanRunConfig) -> GroundingArchitecture:
    ...
    if config.grounding_variant != "tool_call" and config.grounding_variant not in presets:
        raise ValueError(f"Unsupported GroundPlan grounding variant: {config.grounding_variant!r}")
    if config.grounding_variant != "tool_call":
...
@dataclass(frozen=True)
class GroundPlanPipelineResult:
    trajectory: list[list[int]]
    metadata: dict[str, object]
    steps: dict[str, object]
    success: bool
    failure_reason: str | None = None
    debug_data: GroundPlanDebugData | None = None


@dataclass(frozen=True)
class GroundPlanDebugData:
...
def run_groundplan_pipeline(
    ...
    *,
    ...
    if architecture.uses_tool_calls:
        from scripts.methods.groundplan.grounding.tool_pipeline import run_tool_call_pipeline
        ...
        return GroundPlanPipelineResult(
    ...
    try:
        ...
        return GroundPlanPipelineResult(trajectory, metadata, steps, False, "PARSE_FAILED")
    ...
    try:
        ...
        for attempt in range(max_grounding_repairs + 1):
            ...
            if (
                ...
                trajectory = _start_only(map_state, instruction)
                debug_data = GroundPlanDebugData(program=program, scene=scene, grounded=grounded)
                return GroundPlanPipelineResult(
                    trajectory,
                    metadata,
                    steps,
    ...
    try:
        ...
        trajectory = _start_only(map_state, instruction)
        debug_data = GroundPlanDebugData(program=program, scene=scene, grounded=grounded, planned=planned)
        return GroundPlanPipelineResult(
            trajectory,
            metadata,
            steps,
    ...
    return GroundPlanPipelineResult(
        ...
        GroundPlanDebugData(program=program, scene=scene, grounded=grounded, planned=planned),
    )
    ...
    *,
    ...
    try:
        ...
        return GroundPlanPipelineResult(
            ...
            GroundPlanDebugData(program=program, scene=scene, grounded=grounded),
        )
    trajectory = planned.trajectory or _start_only(map_state, instruction)
    metadata["status"] = "success" if planned.status == "success" else str(planned.failure_reason or planned.status)
    return GroundPlanPipelineResult(
