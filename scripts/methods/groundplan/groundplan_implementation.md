...
本文档记录 `scripts/methods/groundplan` 当前版本的完整实现，并补充 SemPathBench benchmark setup 与当前实验结果。结构按照六个部分展开：
...
GroundPlan 的核心思想是：让 LLM 只负责把自然语言翻译成可检查的符号意图，不让 LLM 直接选择 object id、room id、坐标、路径或 planner 参数；随后由确定性 grounding 和 planner 在当前 map 上完成对象绑定、区域构造和路径搜索。

## 1. Overview

### 1.1 端到端流程

当前主入口是 `scripts/methods/groundplan/run.py`。一次 episode 的执行流程是：
...
GroundPlan 自身的 planner/grounding 超参数在 `config.py`：
...
trajectory 必须在 map 的 traversable space 中移动。GroundPlan 的 planner 使用 8-connected movement，也就是上下左右和四个对角方向都可以走，但每一步都需要通过 `can_traverse_between(...)` 检查，避免穿墙、穿过障碍或斜穿不可通行 corner。

当前 GroundPlan 实验主要使用 ProcTHOR val-unseen maps。当前本地 instruction root 中有：
...
因此当前本地 val-unseen instruction 文件总数是 126。当前 GroundPlan summary 中的 50 条结果来自 output root 里已经存在的两个 map：

```text
resources/methods/groundplan/previous/groundplan_previous/GroundPlan/procthor/003_valunseen  25 predictions
resources/methods/groundplan/previous/groundplan_previous/GroundPlan/procthor/006_valunseen  25 predictions
```

这也是为什么你只看 `003_valunseen` 时会看到 25 条，但 `resources/methods/groundplan/previous/groundplan_previous/GroundPlan/summary.json` 会聚合成 50 条。
...
GroundPlan 在 inference 时读取 `<map_leaf>.json`，通过 `load_map_state(map_id)` 得到 map_state，然后 `SceneMap` 会从中构造可通行 grid、room refs、object refs、start ref 和 passage refs。
...
其中 `objects` 是 benchmark annotation 中的目标 object 序列，包含 order、object_id、name、category 和 center。GroundPlan inference 不使用这个字段，只用于 benchmark/evaluation/debug。

`human_expert_trajectory` 是专家轨迹。GroundPlan inference 不使用它，但 evaluator 会用它计算 PLR 和 path-shape preference。
...
```text
resources/methods/groundplan/main_result/<map_id>/instruction_xxxxxx.json
```

对应中间文件是：

```text
resources/methods/groundplan/main_result/<map_id>/instruction_xxxxxx.steps.json
```
...
目前有五个主要 GroundPlan 版本用于比较：

| Run | 结果目录 | 主要变化 | 结论 |
| --- | --- | --- | --- |
| GroundPlan-1st | `resources/methods/groundplan/previous/groundplan_previous/1st_run` | 主要验证 planner 链路 | Planner 基本可以工作；主要瓶颈暴露在 parser/grounding。 |
| GroundPlan-2nd | `resources/methods/groundplan/previous/groundplan_previous/2nd_run` | 修复 parser 和 grounding 基础问题 | Parser/grounding 基本连通，成功率提升，但复杂语义仍不稳定。 |
| GroundPlan-3rd | `resources/methods/groundplan/previous/groundplan_previous/GroundPlan` | 修复 grounding、cost field、path-shape、PLR/evaluation 记录 | PLR 明显提高；剩余主要问题是复杂目标选择和 path-shape。 |
| GroundPlan-Intent-Mixed | `resources/methods/groundplan/previous/groundplan_previous/GroundPlan_intent` | 初始 parser 输出 JSON intent，但 grounding repair 曾混入 DSL repair | HCS/OSS 提升，但不是纯 JSON 消融。 |
| GroundPlan-Intent-PureJSON | `resources/methods/groundplan/previous/GroundPlan_intent_pure` | 初始 parsing 和 grounding repair 都保持 JSON intent | 当前最好结果，更干净地说明 JSON intent 比直接 DSL 更稳。 |
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| GroundPlan-1st | 25 | 0.581 | 0.300 | 0.607 | 0.201 | 17 | 0.623 | 0.265 | 0.680 | 0.179 | 8 | 0.491 | 0.375 | 0.452 | 0.247 |
| GroundPlan-2nd | 50 | 0.635 | 0.490 | 0.646 | 0.338 | 34 | 0.707 | 0.515 | 0.717 | 0.367 | 16 | 0.482 | 0.438 | 0.495 | 0.276 |
| GroundPlan-3rd | 50 | 0.910 | 0.567 | 0.723 | 0.417 | 35 | 0.910 | 0.629 | 0.720 | 0.481 | 15 | 0.909 | 0.422 | 0.731 | 0.266 |
| GroundPlan-Intent-Mixed | 50 | 0.862 | 0.683 | 0.780 | 0.492 | 35 | 0.868 | 0.714 | 0.798 | 0.533 | 15 | 0.850 | 0.611 | 0.739 | 0.396 |
| GroundPlan-Intent-PureJSON | 50 | 0.863 | 0.723 | 0.783 | 0.519 | 35 | 0.864 | 0.743 | 0.798 | 0.557 | 15 | 0.862 | 0.678 | 0.748 | 0.432 |

### 3.3 SCS 细分指标

当前 `GroundPlan-3rd` 的 SCS detail average：
...
当前 `GroundPlan-Intent-PureJSON` 的 SCS detail average：
...
这说明 planner 链路、fallback、cost field 和 evaluation 记录修复以后，GroundPlan 已经能稳定输出长度合理的路线。
...
```text
GroundPlan-3rd path_shape overall: 0.324
...
当前最好结果是 `GroundPlan-Intent-PureJSON`：
...
Grounding 和 planner 的核心逻辑与 GroundPlan-3rd 保持一致，所以这是一个比较干净的 parser representation ablation。
...
```text
scripts/methods/groundplan/parse/gemini_api_program_prompt.md
```
...
GroundPlan 还有一个专门的 unknown object category alias resolver。
...
当前 GroundPlan 已经把 parser、grounding 和 planner 明确拆开，但仍有几个重要边界：
