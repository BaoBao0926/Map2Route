"""LLM-assisted learned aliases for unsupported GroundPlan object categories."""
...
from scripts.methods.groundplan.alias_utils import (
...
def resolve_unknown_entity_alias(
    *,
    ...
    prompt = f"""
You are helping maintain GroundPlan object category aliases.
...
""".strip()
    text, meta = llm_client.response_text(
        system_prompt="You map unknown GroundPlan object categories to supported object categories. Return JSON only.",
        user_prompt=prompt,
        cache_namespace="groundplan_entity_alias_resolver",
    )
    try:
