"""JSON intent-graph parser for GroundPlan.
...
def llm_parse_intent_instruction(
    ...
    *,
    ...
    text, meta = llm_client.response_text(
        system_prompt="You are the SemPathBench GroundPlan intent parser. Return JSON only.",
        user_prompt=user_prompt,
        cache_namespace="groundplan_intent_parser",
    )
...
""".strip()
    text, meta = llm_client.response_text(
        system_prompt="You repair SemPathBench GroundPlan intent JSON. Return JSON only.",
        user_prompt=repair_prompt,
        cache_namespace=f"groundplan_intent_parser_parse_repair_v{attempt}",
    )
    try:
        intent = parse_intent_json(text)
...
GroundPlan DSL, but deterministic grounding failed. Rewrite the JSON intent
...
""".strip()
    text, meta = llm_client.response_text(
        system_prompt="You repair SemPathBench GroundPlan intent JSON after deterministic grounding errors. Return JSON only.",
        user_prompt=repair_prompt,
        cache_namespace=f"groundplan_intent_parser_grounding_repair_v{attempt}",
    )
    try:
        intent = parse_intent_json(text)
