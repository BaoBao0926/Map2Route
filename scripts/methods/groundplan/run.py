#!/usr/bin/env python3
"""Run the GroundPlan SemPathBench method."""
...
from scripts.methods.groundplan.artifacts import save_groundplan_debug_artifacts
from scripts.methods.groundplan.constraint_evaluation import evaluate_constraint_grounding, write_grounding_report
from scripts.methods.groundplan.llm_client import GeminiClient, configured_model
from scripts.methods.groundplan.pipeline import GroundPlanRunConfig, run_groundplan_pipeline
from scripts.methods.groundplan.tool_efficiency import write_tool_call_efficiency_report
...
def verbose_print(verbose: bool, message: str) -> None:
    if verbose:
        print(f"[groundplan] {message}", flush=True)
...
def _prediction_artifacts_complete(
    ...
    *,
    ...
    if not isinstance(record, Mapping) or not isinstance(steps_payload, Mapping):
        return False
    groundplan = record.get("groundplan")
    if (
        resume_execution_errors
        and isinstance(groundplan, Mapping)
        and groundplan.get("status") == "EXECUTION_ERROR_WORST_CASE"
    ):
        return False
...
def build_prediction_record(
    ...
    *,
    ...
    groundplan_metadata: dict[str, object],
    ...
    return {
        "version": 1,
        "method": METHOD_NAME,
        "prediction_id": f"GroundPlan_{uuid.uuid4().hex[:8]}",
        "map_id": map_id,
        "scene_id": scene_id,
...
def build_all(
    ...
    *,
    ...
    if grounding_variant == "tool_call" and worker_count != 1:
        print(
            "[GroundPlan][warning] ToolCall forces --workers 1 after a prior "
    ...
    def run_one(index: int, instruction_file: Path) -> dict[str, object]:
        ...
        if not overwrite and _prediction_artifacts_complete(
            ...
            verbose_print(verbose, f"[{index}/{total}] running GroundPlan pipeline")
            replay_selection = (
            ...
            pipeline_result = run_groundplan_pipeline(
                map_state,
                instruction,
                config=config,
            ...
            record = build_prediction_record(
                ...
                groundplan_metadata=pipeline_result.metadata,
                metrics=metrics,
            ...
            try:
                ...
                groundplan = record.get("groundplan")
                if isinstance(groundplan, dict):
                    groundplan["trajectory_image_error"] = image_error
                print(
                    f"[GroundPlan][warning] trajectory image failed for "
                    f"{scene_id}/{instruction_id}: {image_error}; continuing.",
                    flush=True,
                )
            if save_debug_artifacts:
                artifact_paths = save_groundplan_debug_artifacts(
                ...
                record["groundplan_debug_artifacts"] = artifact_paths
                groundplan = record.get("groundplan")
                if isinstance(groundplan, dict):
                    groundplan["debug_artifacts"] = artifact_paths
            ...
            groundplan = record.get("groundplan")
            if isinstance(groundplan, dict):
                groundplan["runtime_seconds"] = runtime_seconds
        ...
        result["_groundplan_index"] = index
        return result
        ...
        try:
            ...
            try:
                ...
                metrics = worst_case_metrics_for_evaluation_error(
                    ...
                    instruction,
                    map_state,
                    error=f"GroundPlan unexpected episode failure: {error_text}",
                )
                failure = {
                    "policy": "worst_case",
                    "stage": "run_groundplan_pipeline",
                    "error": error_text,
                ...
                result["_groundplan_index"] = index
                print(
                    f"[GroundPlan][warning] {scene_id}/{instruction_id} failed with "
    ...
    if worker_count == 1:
        ...
        with ThreadPoolExecutor(max_workers=worker_count) as executor:
            ...
            for future in as_completed(futures):
                ...
                original_index = result.pop("_groundplan_index", completed)
                results.append(result)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the GroundPlan SemPathBench method.")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "With --overwrite omitted, rerun saved episodes whose GroundPlan "
...
def _write_grounding_evaluation_report(output_root: Path) -> None:
    ...
    for prediction_path in sorted(output_root.rglob("instruction_*.json")):
        ...
        try:
            ...
            continue
        groundplan = prediction.get("groundplan") if isinstance(prediction, Mapping) else None
        report = groundplan.get("grounding_evaluation") if isinstance(groundplan, Mapping) else None
...
def _validate_ablation_source(args: argparse.Namespace, source_root: Path) -> None:
    ...
    if missing:
        ...
        raise FileNotFoundError(
            ...
            f"examples: {examples}. Run the canonical GroundPlan method first."
        )
...
def run_ablation_suite(args: argparse.Namespace) -> Path:
    ...
    for case in AUTOMATIC_ABLATION_CASES:
        ...
        print(
            f"[groundplan-ablation] case={name} offline_replay={display_path(source_root)}",
            flush=True,
        )
