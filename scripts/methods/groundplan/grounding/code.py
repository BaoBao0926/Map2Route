"""Executable, planner-free grounding code for GroundPlan.
...
from scripts.methods.groundplan.grounding.grounder import (
    GroundingFailure,
    _eval_expr,
...
from scripts.methods.groundplan.grounding.semantic_api import (
...
def execute_grounding_code(
    ...
    *,
    ...
    try:
        compiled = compile(tree, "<groundplan-grounding>", "exec")
...
def llm_refine_grounding_code(
    ...
    *,
    ...
    text, metadata = llm_client.response_text(
        system_prompt="You refine planner-free GroundPlan grounding code. Return Python code only.",
        user_prompt=prompt,
        cache_namespace=f"groundplan_grounding_code_refine_v{attempt}",
    )
...
def llm_generate_direct_grounding_code(
    ...
    *,
    ...
    text, metadata = llm_client.response_text(
        system_prompt="You generate planner-free GroundPlan grounding code. Return Python code only.",
        user_prompt=prompt,
        cache_namespace="groundplan_direct_cap_grounding_code",
    )
...
def llm_repair_grounding_code(
    ...
    *,
    ...
    text, metadata = llm_client.response_text(
        system_prompt="You repair planner-free GroundPlan grounding code from execution feedback. Return Python code only.",
        user_prompt=prompt,
        cache_namespace=f"groundplan_grounding_code_repair_v{attempt}",
    )
...
_DEFAULT_PROMPT = """You write GroundPlan grounding code only. The code executes against a read-only semantic map and must define `ground()` with no arguments. It must return `task(segments, bindings)`.
