...
class AblationRunnerTests(unittest.TestCase):
    def test_resume_only_skips_complete_prediction_and_steps(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            prediction["grounding2route"] = {"status": "EXECUTION_ERROR_WORST_CASE"}
            prediction_path.write_text(json.dumps(prediction), encoding="utf-8")
            self.assertTrue(
                _prediction_artifacts_complete(prediction_path, steps_path)
            )
    ...
    def test_unexpected_episode_error_writes_worst_case_and_continues(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                patch(
                    "scripts.methods.grounding2route.run.iter_instruction_files",
                    return_value=instruction_files,
                ),
                patch(
                    "scripts.methods.grounding2route.run.load_instruction",
                    return_value=instruction,
                ),
                patch(
                    "scripts.methods.grounding2route.run.map_id_from_instruction_path",
                    return_value="procthor/001_valunseen",
                ),
                patch(
                    "scripts.methods.grounding2route.run.instruction_id_from_payload",
                    side_effect=lambda source, _payload: source.stem,
                ),
                patch(
                    "scripts.methods.grounding2route.run.load_map_state",
                    return_value={"grid_size": 4, "layers": {"occupancy": [[0] * 4 for _ in range(4)]}},
                ),
                patch(
                    "scripts.methods.grounding2route.run.run_grounding2route_pipeline",
                    side_effect=[
                ...
                patch(
                    "scripts.methods.grounding2route.run.worst_case_metrics_for_evaluation_error",
                    return_value=worst_metrics,
                ),
                patch(
                    "scripts.methods.grounding2route.run._evaluate",
                    return_value=(metrics, None),
                ),
                patch(
                    "scripts.methods.grounding2route.run.save_trajectory_image_for_record",
                    return_value=None,
                ),
            ):
            ...
            self.assertEqual(
                first_record["grounding2route"]["status"],
