"""Debug artifact writers for Grounding2Route runs."""
...
from scripts.methods.grounding2route.ir import GPExpr, GPProgramSpec
from scripts.methods.grounding2route.pipeline import Grounding2RouteDebugData
from scripts.methods.grounding2route.planner.semantic_astar import (
    _compile_hard,
    _compile_soft,
    _goal_mask,
    _traversable_mask,
...
def save_grounding2route_debug_artifacts(
    *,
    output_path: Path,
    ...
    debug_data: Grounding2RouteDebugData | None,
) -> dict[str, object]:
    ...
    grounded_program_path.write_text(
        "# Generated Grounding2Route grounded IR for inspection only.\n"
...
def _grounding_trace_source(
    ...
    lines = [
        "# Generated Grounding2Route grounding trace for inspection only.",
        "# The real method does not execute this file. It runs",
        "# scripts/methods/grounding2route/grounding/grounder.py as a deterministic interpreter.",
        "",
        "env = {}",
...
def _write_cost_map(path: Path, debug_data: Grounding2RouteDebugData, *, scale: int = 3) -> bool:
    try:
        from PIL import Image, ImageDraw
    except ImportError as exc:
        raise RuntimeError("Pillow is required to write Grounding2Route cost maps.") from exc
