# Grounding2Route SemPathBench

Grounding2Route is a benchmark-native SemPathBench method. It follows the design in
`design.md`:
...
python scripts/methods/grounding2route/run.py \
  --set valunseen \
  --output-root resources/methods/grounding2route/ablation/05_global_vs_sequential_planning/global_planner \
  --replay-grounding-code-root resources/methods/grounding2route/main_result \
...
python scripts/methods/grounding2route/run.py \
  --set valunseen \
  --output-root resources/methods/grounding2route/main_result \
  --replay-grounding-code-root resources/methods/grounding2route/main_result \
  --planner-mode global_layered_dp \
  --max-expansions 0 \
...
python scripts/methods/grounding2route/run.py \
  ...
  --output-root resources/methods/grounding2route/previous/experiments/incomplete_schema_no_repair \
  --overwrite --verbose
```
...
python scripts/methods/grounding2route/run.py \
  ...
  --llm-cache-root resources/methods/grounding2route/main_result/_llm_cache \
...
```bash
python scripts/methods/grounding2route/planner/debug_annotation_planner.py \
  --set valunseen \
  --limit 25 \
  --overwrite
```
...
then calls the current Grounding2Route planner. Outputs are written to:

```text
resources/methods/grounding2route/ablation/06_planner_over_oracle_grounding/ours
```
...
`Grounding2RouteRunConfig` and records them under `planner_config` in the
Grounding2Route metadata. CLI overrides are `--relative-cost-mode` and
...
```bash
python scripts/methods/grounding2route/relative_cost_experiment.py \
  --limit-relative 20 \
...
behavior with GT information; the normal Grounding2Route parser/grounder path does
not receive these annotation fields.

Run the first 25 val-unseen instructions as a normal Grounding2Route experiment:
...
```text
resources/methods/grounding2route/main_result/summary.json
resources/methods/grounding2route/main_result/<map_id>/<instruction_id>.json
resources/methods/grounding2route/main_result/<map_id>/<instruction_id>.steps.json
resources/methods/grounding2route/main_result/<map_id>/<instruction_id>.png
resources/methods/grounding2route/main_result/<map_id>/<instruction_id>.artifacts/
```
...
| `--output-root PATH` | Output root, default `resources/methods/grounding2route/main_result`. |
