...
def write_tool_call_efficiency_report(output_root: Path) -> Path:
    ...
    for path in sorted(output_root.rglob("instruction_*.json")):
        ...
        try:
            ...
            continue
        grounding2route = record.get("grounding2route") if isinstance(record, Mapping) else None
        stats = grounding2route.get("grounding_efficiency") if isinstance(grounding2route, Mapping) else None
