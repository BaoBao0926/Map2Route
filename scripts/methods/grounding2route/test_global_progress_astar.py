"""Regression tests for Grounding2Route global position-progress A*."""
...
from scripts.methods.grounding2route.pipeline import Grounding2RouteRunConfig
from scripts.methods.grounding2route.planner.semantic_astar import _compile_soft, plan_grounded_program
...
class PlannerDefaultTests(unittest.TestCase):
    ...
    def test_planner_api_defaults_to_sequential(self) -> None:
        ...
        with patch("scripts.methods.grounding2route.planner.semantic_astar._compile_soft", side_effect=zero_soft):
...
class GlobalProgressAStarTests(unittest.TestCase):
    ...
    def test_segment_and_spatial_scopes_gate_soft_cost(self) -> None:
        ...
        with patch("scripts.methods.grounding2route.planner.semantic_astar._compile_soft", wraps=_compile_soft) as compile_soft:
    ...
    def test_astar_matches_zero_heuristic_dijkstra(self) -> None:
        ...
        with patch("scripts.methods.grounding2route.planner.semantic_astar._compile_soft", side_effect=zero_soft):
            ...
            with patch("scripts.methods.grounding2route.planner.semantic_astar._reverse_distance_field", side_effect=lambda scene, base, mask: np.zeros((scene.grid_size, scene.grid_size))):
    ...
    def test_relative_ratio_cost_rewards_stronger_satisfying_margin(self) -> None:
        ...
        with patch(
            "scripts.methods.grounding2route.planner.semantic_astar._clearance_field",
...
class GlobalLayeredDPTests(unittest.TestCase):
    def test_selects_globally_better_intermediate_arrival(self) -> None:
        ...
        program = GroundedProgram((segment("s1", first), segment("s2", final)), {})
        with patch(
            "scripts.methods.grounding2route.planner.semantic_astar._compile_soft",
