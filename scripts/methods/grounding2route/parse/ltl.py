...
Boolean temporal operators.  Both parts compile to the same Grounding2Route IR used
...
from scripts.methods.grounding2route.parse.direct_id import direct_id_to_program
...
def llm_parse_ltl_instruction(
    ...
    *,
    ...
    text, meta = llm_client.response_text(
        ...
        user_prompt=prompt,
        cache_namespace="grounding2route_ltl_catalog_v1",
    )
    ...
    try:
        ...
        repaired, repair_meta = llm_client.response_text(
            system_prompt="Repair the restricted Grounding2Route LTL JSON. Return JSON only.",
            user_prompt=(
                f"{prompt}\n\n## Validation repair\n\n"
                f"Validation error:\n{exc}\n\nPrevious response:\n{text}\n\n"
                "Return one complete corrected JSON object and nothing else."
            ),
            cache_namespace="grounding2route_ltl_catalog_parse_repair_v1",
        )
        try:
...
def ltl_to_program(
    ...
    *,
    ...
    """Validate restricted LTL and compile its propositions to Grounding2Route IR."""
    ...
    program = GPProgramSpec(
        ...
        parser_mode="ltl",
        diagnostics=(
            "restricted_ltl_compiled_to_grounding2route_ir",
