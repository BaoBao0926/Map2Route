"""Instruction parsing for Grounding2Route."""
...
from scripts.methods.grounding2route.parse.api_program import (
    APIProgramParseError,
    llm_parse_api_program_instruction,
)
from scripts.methods.grounding2route.parse.intent import (
    IntentParseError,
...
from scripts.methods.grounding2route.parse.ltl import LTLParseError, llm_parse_ltl_instruction
from scripts.methods.grounding2route.ir import GPProgramSpec


class Grounding2RouteParseError(RuntimeError):
...
def parse_instruction(
    ...
    *,
    ...
    if mode not in {"intent", "api", "direct_id", "ltl"}:
        raise Grounding2RouteParseError(
            (
                "Grounding2Route parser supports intent, api, direct_id, and ltl modes; "
    ...
    if llm_client is None:
        raise Grounding2RouteParseError(
            "llm_client is required for Grounding2Route parser mode.",
    ...
    try:
        ...
        raise Grounding2RouteParseError(str(exc), metadata) from exc
