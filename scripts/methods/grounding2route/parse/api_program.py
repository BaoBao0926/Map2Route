"""Restricted API-program parser for Grounding2Route."""
...
def llm_parse_api_program_instruction(
    ...
    *,
    ...
    text, meta = llm_client.response_text(
        system_prompt="You are the SemPathBench Grounding2Route API-program parser. Return code only.",
        user_prompt=user_prompt,
        cache_namespace="grounding2route_api_program_parser",
    )
...
""".strip()
    text, meta = llm_client.response_text(
        system_prompt="You repair SemPathBench Grounding2Route restricted API programs. Return code only.",
        user_prompt=repair_prompt,
        cache_namespace=f"grounding2route_api_program_parse_repair_v{attempt}",
    )
    try:
...
""".strip()
    text, meta = llm_client.response_text(
        system_prompt="You repair SemPathBench Grounding2Route API programs after grounding errors. Return code only.",
        user_prompt=repair_prompt,
        cache_namespace=f"grounding2route_api_program_grounding_repair_v{attempt}",
    )
    try:
